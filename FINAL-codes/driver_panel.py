...
def init_ui(self):
    ...
    self.logout_button.setStyleSheet("background-color: #2196F3; color: white; font-weight: bold; border-radius: 5px; padding: 7px 18px;")
    ...
    self.fares_table.horizontalHeader().setStyleSheet("QHeaderView::section { background-color: #1976D2; color: white; font-weight: bold; }")
    ...
    self.feedbacks_table.horizontalHeader().setStyleSheet("QHeaderView::section { background-color: #1976D2; color: white; font-weight: bold; }")
...
def load_assigned_vehicle(self):
    ...
    LIMIT 1
    """
    result = self.db_manager.execute_query(query, (self.driver_id,))
